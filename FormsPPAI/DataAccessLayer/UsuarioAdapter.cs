...
namespace Dashbord.DataAccessLayer {
	public static class UsuarioAdapter {
		public static DataTable ReadUsuario(string username, string password) => GeneralAdapter.Query("SELECT e.* FROM Usuario u JOIN Empleado e ON u.id_empleado = e.dni WHERE u.nombre = @username AND u.contraseña = @password", new Dictionary<string, string>() { { "@username", username }, { "@password", password } });
